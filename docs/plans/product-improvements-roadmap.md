...
- 2026-03-24: Batch 6 command-surface split landed for the post-launch cleanup program.
  - `src-tauri/src/commands/mod.rs` now acts as a thin index layer instead of the main command implementation hotspot.
  - `src-tauri/src/commands/registry.rs` is now the single source of truth for Tauri command registration, and both the build script and the permission-manifest test read from that registry.
  - The phase introduced `docs/adr/0005-tauri-command-registry-and-domain-modules.md` to capture the registry/domain-module decision and the temporary legacy-implementation tradeoff.
  - Batch 7 is now unblocked because the command spine no longer depends on a single giant registration block in `src/lib.rs`.
  - Closeout verification on the final Batch 6 tree: `pnpm check:workstation-preflight`, `pnpm check:workflow-drift`, `pnpm ui:gate:static`, `pnpm test`, `pnpm test:coverage`, `pnpm exec playwright test tests/ui/app-shell.spec.ts tests/ui/app-shell-responsive.spec.ts tests/ui/workspace-performance.spec.ts`, `VITE_ASSISTSUPPORT_ENABLE_ADMIN_TABS=1 pnpm exec playwright test tests/ui/admin-shell.spec.ts`, `pnpm test:e2e:smoke`, `pnpm ui:gate:regression`, `pnpm perf:workspace`, `pnpm perf:bundle`, `pnpm perf:build`, `pnpm perf:assets`, `pnpm perf:memory`, `pnpm perf:lhci`, `pnpm test:ci`, `cd src-tauri && cargo test --test permission_manifest`, `pnpm test:security-regression`, `pnpm search-api:test`, `ENVIRONMENT=production ASSISTSUPPORT_API_KEY=local-smoke-key ASSISTSUPPORT_RATE_LIMIT_STORAGE_URI=redis://127.0.0.1:6380/0 pnpm search-api:smoke`, `pnpm search-api:openapi:check`, and `pnpm git:guard:all`. Lighthouse still reports the existing non-blocking SEO warning at `0.82` against the `0.90` threshold.
- 2026-03-24: Batch 7 database-facade split landed for the post-launch cleanup program.
  - `src-tauri/src/db/mod.rs` is now a stable facade and compatibility re-export layer instead of the main persistence implementation hotspot.
  - The database implementation now lives behind domain-owned internal stores: `bootstrap.rs`, `migrations.rs`, `runtime_state_store.rs`, `draft_store.rs`, `knowledge_store.rs`, `analytics_ops_store.rs`, and `workspace_store.rs`, alongside the existing `job_store.rs`, `executor.rs`, and `path_helpers.rs`.
  - The phase introduced `docs/adr/0006-database-facade-and-internal-stores.md` to capture the stable-facade decision, the “no new abstraction layer” rule, and the compatibility tradeoff of keeping the public DB record types re-exported from `crate::db::*`.
  - Batch 8 is now unblocked because search-api simplification no longer depends on a single multi-thousand-line SQLite implementation file.
  - Closeout verification on the final Batch 7 tree: `pnpm check:workstation-preflight`, `pnpm check:workflow-drift`, `pnpm ui:gate:static`, `pnpm test`, `pnpm test:coverage`, `pnpm exec playwright test tests/ui/app-shell.spec.ts tests/ui/app-shell-responsive.spec.ts tests/ui/workspace-performance.spec.ts`, `VITE_ASSISTSUPPORT_ENABLE_ADMIN_TABS=1 pnpm exec playwright test tests/ui/admin-shell.spec.ts`, `pnpm test:e2e:smoke`, `pnpm ui:gate:regression`, `pnpm perf:workspace`, `pnpm perf:bundle`, `pnpm perf:build`, `pnpm perf:assets`, `pnpm perf:memory`, `pnpm perf:lhci`, `pnpm test:ci`, `cd src-tauri && cargo test`, `pnpm test:security-regression`, `pnpm search-api:test`, `ENVIRONMENT=production ASSISTSUPPORT_API_KEY=local-smoke-key ASSISTSUPPORT_RATE_LIMIT_STORAGE_URI=redis://127.0.0.1:6380/0 pnpm search-api:smoke`, `pnpm search-api:openapi:check`, `pnpm git:guard:all`, `cd src-tauri && cargo test --test db_encrypted_roundtrip`, `cd src-tauri && cargo test --test data_migration`, `cd src-tauri && cargo test --test kb_pipeline`, `cd src-tauri && cargo test --test kb_disk_ingestion`, and `cd src-tauri && cargo test --test namespace_consistency`. Lighthouse still reports the existing non-blocking SEO warning at `0.82` against the `0.90` threshold.
- 2026-03-24: Batch 8 search-api simplification and whole-app review landed for the post-launch cleanup program.
  - The live search sidecar now runs on the single adaptive runtime path that the desktop app already used in practice, while `/health`, `/ready`, `/search`, `/feedback`, and `/stats` remained stable for callers.
  - `/ready` now reflects the surviving dependency set instead of carrying reranker-oriented readiness expectations, and the decision is captured in `docs/adr/0007-search-api-runtime-path-simplification.md`.
  - Search diagnostics in Knowledge stayed behaviorally stable while the service docs and generated OpenAPI contract were updated to describe the narrower runtime shape.
  - Whole-codebase review across frontend, Tauri/Rust, Python service, security, and test/CI found no open P0/P1 blockers on the final tree. The remaining cleanup work is a Phase 9 deletion docket.
  - Closeout verification on the final Batch 8 tree: `pnpm check:workstation-preflight`, `pnpm check:workflow-drift`, `pnpm ui:gate:static`, `pnpm test`, `pnpm test:coverage`, `pnpm exec playwright test tests/ui/app-shell.spec.ts tests/ui/app-shell-responsive.spec.ts tests/ui/workspace-performance.spec.ts`, `VITE_ASSISTSUPPORT_ENABLE_ADMIN_TABS=1 pnpm exec playwright test tests/ui/admin-shell.spec.ts`, `pnpm test:e2e:smoke`, `pnpm ui:gate:regression`, `pnpm perf:workspace`, `pnpm perf:bundle`, `pnpm perf:build`, `pnpm perf:assets`, `pnpm perf:memory`, `pnpm perf:lhci`, `pnpm test:ci`, `cd src-tauri && cargo test`, `pnpm test:security-regression`, `pnpm search-api:test`, `ENVIRONMENT=production ASSISTSUPPORT_API_KEY=local-smoke-key ASSISTSUPPORT_RATE_LIMIT_STORAGE_URI=redis://127.0.0.1:6380/0 pnpm search-api:smoke`, `pnpm search-api:openapi:check`, `BASE_URL=http://127.0.0.1:3000 AUTH_TOKEN=local-smoke-key pnpm perf:api`, `env -u DATABASE_URL pnpm perf:db`, and `pnpm git:guard:all`. The final perf closeout used a temporary local PostgreSQL 16 instance on `127.0.0.1:5432`, a minimal local search schema for the BM25 query path, and a temporary local WSGI sidecar on `127.0.0.1:3000`. Lighthouse still reports the existing non-blocking SEO warning at `0.82` against the `0.90` threshold.
- 2026-03-24: Batch 9 deletion and baseline review landed for the post-launch cleanup program.
  - Deleted retired queue, pilot, and search wrapper screens and removed the standalone follow-up compatibility surface after Queue fully absorbed the history/template workflows.
  - Deleted `useFeatureOps.ts` and aligned the surviving shell/header/sidebar/tab-bar surfaces to the current product tab set instead of preserving retired tab values in shared UI types.
  - Retired the final search-api compatibility residue from Batch 8: `reranker.py` is gone, `rerank_time_ms` is no longer part of the checked-in contract, and score-fusion helpers now reflect the surviving adaptive runtime path.
  - The phase introduced `docs/adr/0008-compatibility-surface-retirement.md` to record the compatibility-retirement rules, explicit non-goals, and the carry-forward items for the next backend convergence wave.
  - Whole-codebase post-implementation review on the final Batch 9 tree found no open P0/P1 blockers on live code paths.
  - Performance baselines were reviewed but not reset. Bundle size moved to `1,118,425` bytes (`+8.69%` vs baseline) and build time moved to `3282ms` (`+13.96%` vs baseline), both inside the locked thresholds, so `.perf-baselines/*` remains unchanged.
  - Closeout verification on the final Batch 9 tree: `pnpm check:workstation-preflight`, `pnpm check:workflow-drift`, `pnpm ui:gate:static`, `pnpm test`, `pnpm test:coverage`, `pnpm exec playwright test tests/ui/app-shell.spec.ts tests/ui/app-shell-responsive.spec.ts tests/ui/workspace-performance.spec.ts`, `VITE_ASSISTSUPPORT_ENABLE_ADMIN_TABS=1 pnpm exec playwright test tests/ui/admin-shell.spec.ts`, `pnpm test:e2e:smoke`, `pnpm ui:gate:regression`, `pnpm perf:workspace`, `pnpm perf:bundle`, `pnpm perf:build`, `pnpm perf:assets`, `pnpm perf:memory`, `pnpm perf:lhci`, `pnpm test:ci`, `pnpm test:security-regression`, `pnpm search-api:test`, `ENVIRONMENT=production ASSISTSUPPORT_API_KEY=local-smoke-key ASSISTSUPPORT_RATE_LIMIT_STORAGE_URI=redis://127.0.0.1:6380/0 pnpm search-api:smoke`, `pnpm search-api:openapi:check`, `BASE_URL=http://127.0.0.1:3000 AUTH_TOKEN=local-smoke-key pnpm perf:api`, `DATABASE_URL=postgresql://127.0.0.1:5432/postgres pnpm perf:db`, and `pnpm git:guard:all`. The final perf closeout used a temporary local Postgres 16 instance on `127.0.0.1:5432` with a minimal BM25 table plus a temporary local loopback sidecar on `127.0.0.1:3000`. Lighthouse still reports the existing non-blocking SEO warning at `0.82` against the `0.90` threshold.
- 2026-03-25: Batch 10 internal convergence groundwork landed for the post-launch cleanup program.
  - More command ownership moved out of the old compatibility core and into the real domain modules, including analytics/ops, Jira, draft, KB, and early model/runtime command surfaces.
  - Another workspace/frontend import cluster moved from the broad `src/types/index.ts` barrel to domain-owned type modules, reducing the risk of keeping the barrel around for “just one more wave.”
  - Groundwork verification passed on the converged tree: `cd src-tauri && cargo test --test permission_manifest`, `cd src-tauri && cargo test --test command_contracts`, `cd src-tauri && cargo test`, `cd ~/AssistSupport && pnpm exec vitest run src/features/workspace/TicketWorkspaceRail.test.tsx src/features/workspace/workspaceAssistant.test.ts src/features/workspace/useWorkspaceDraftState.test.ts src/components/Analytics/AnalyticsTab.test.tsx`, and `cd ~/AssistSupport && pnpm exec tsc --noEmit`.
- 2026-03-25: Batch 11 final convergence and hardening closeout landed for the post-launch cleanup program.
  - Deleted `src-tauri/src/commands/legacy_commands.rs` after the remaining runtime/model ownership moved into `model_commands.rs` and the already-established domain command modules.
  - Deleted `src/types/index.ts` after production imports converged on domain-owned type modules and added structural guards so the broad barrel cannot silently return.
  - Narrowed `src-tauri/src/db/mod.rs` by removing the dead broad store re-export surface while preserving `Database`, `DbError`, schema version `15`, and the intentionally stable DB facade.
  - Removed the final live-code `fusion_strategy` residue from the search path and perf harnesses so the surviving adaptive-only contract is reflected in runtime tooling as well as docs.
  - Whole-codebase post-implementation review on the final Batch 11 tree found no open P0/P1 blockers on live code paths.
  - Closeout verification on the final Batch 11 tree: `pnpm check:workstation-preflight`, `pnpm check:workflow-drift`, `pnpm ui:gate:static`, `pnpm test`, `pnpm test:coverage`, `pnpm exec playwright test tests/ui/app-shell.spec.ts tests/ui/app-shell-responsive.spec.ts tests/ui/workspace-performance.spec.ts`, `VITE_ASSISTSUPPORT_ENABLE_ADMIN_TABS=1 pnpm exec playwright test tests/ui/admin-shell.spec.ts`, `pnpm test:e2e:smoke`, `pnpm ui:gate:regression`, `pnpm perf:workspace`, `pnpm perf:bundle`, `pnpm perf:build`, `pnpm perf:assets`, `pnpm perf:memory`, `pnpm perf:lhci`, `pnpm test:ci`, `cd src-tauri && cargo test`, `pnpm test:security-regression`, `pnpm search-api:test`, `ENVIRONMENT=production ASSISTSUPPORT_API_KEY=local-smoke-key ASSISTSUPPORT_RATE_LIMIT_STORAGE_URI=redis://127.0.0.1:6380/0 pnpm search-api:smoke`, `pnpm search-api:openapi:check`, `BASE_URL=http://127.0.0.1:3000 AUTH_TOKEN=local-smoke-key pnpm perf:api`, `ASSISTSUPPORT_DB_HOST=127.0.0.1 ASSISTSUPPORT_DB_PORT=5432 ASSISTSUPPORT_DB_USER=assistsupport_dev ASSISTSUPPORT_DB_NAME=assistsupport_dev pnpm perf:db`, and `pnpm git:guard:all`. Focused structural checks also passed: `cd src-tauri && cargo test --test permission_manifest`, `cd src-tauri && cargo test --test command_contracts`, `cd src-tauri && cargo test --test db_facade_structure`, `cd ~/AssistSupport && pnpm exec tsc --noEmit`, `cd ~/AssistSupport && pnpm exec vitest run src/types/importPolicy.test.ts src/features/workspace/workspaceAssistant.test.ts src/features/workspace/useWorkspaceDraftState.test.tsx src/components/Analytics/AnalyticsTab.test.tsx`, `cd ~/AssistSupport && pnpm search-api:openapi:check`, and `git diff --check`.
  - The final perf closeout again used a temporary local Postgres 16 instance on `127.0.0.1:5432`, a minimal KB-search schema for the perf query path, and a temporary loopback WSGI sidecar on `127.0.0.1:3000`. Those temporary services were shut back down after verification. Lighthouse still reports the existing non-blocking SEO warning at `0.82` against the `0.90` threshold.
- 2026-03-25: Batch 12 final module right-sizing and release-readiness cleanup landed as the final refactor-program closeout wave.
  - `src-tauri/src/commands/model_commands.rs` is now a thinner command/controller surface, with helper-heavy runtime ownership moved into `model_runtime.rs`, `embedding_runtime.rs`, `download_runtime.rs`, `ocr_runtime.rs`, and `decision_tree_runtime.rs`.
  - `src-tauri/src/db/mod.rs` now acts as a narrower facade and re-export index while the remaining public DB record types live in `types_runtime.rs`, `types_drafts.rs`, `types_knowledge.rs`, `types_analytics_ops.rs`, and `types_workspace.rs`.
  - `src-tauri/src/commands/mod.rs` no longer blanket re-exports whole command modules; only the targeted contract types that real consumers still need remain re-exported.
  - `docs/runbooks/search-api-local-deployment.md` now captures the reusable local setup for `perf:api` and `perf:db`, including temporary Postgres setup, minimal search schema seeding, loopback WSGI startup, and shutdown steps.
  - Validation completed on the final tree: `cd src-tauri && cargo test --test command_module_structure --test command_contracts --test db_facade_structure`, `cd src-tauri && cargo test --test permission_manifest`, `cd src-tauri && cargo test`, `cd ~/AssistSupport && pnpm exec tsc --noEmit`, `cd ~/AssistSupport && git diff --check`, `cd ~/AssistSupport && pnpm test`, `cd ~/AssistSupport && pnpm search-api:test`, `cd ~/AssistSupport && pnpm search-api:openapi:check`, `cd ~/AssistSupport && pnpm test:ci`, `cd ~/AssistSupport && pnpm check:workstation-preflight`, `cd ~/AssistSupport && pnpm check:workflow-drift`, `cd ~/AssistSupport && pnpm test:security-regression`, `cd ~/AssistSupport && pnpm ui:gate:static`, `cd ~/AssistSupport && pnpm test:coverage`, `cd ~/AssistSupport && pnpm exec playwright test tests/ui/app-shell.spec.ts tests/ui/app-shell-responsive.spec.ts tests/ui/workspace-performance.spec.ts`, `cd ~/AssistSupport && VITE_ASSISTSUPPORT_ENABLE_ADMIN_TABS=1 pnpm exec playwright test tests/ui/admin-shell.spec.ts`, `cd ~/AssistSupport && pnpm test:e2e:smoke`, `cd ~/AssistSupport && pnpm ui:gate:regression`, `cd ~/AssistSupport && pnpm perf:bundle`, `cd ~/AssistSupport && pnpm perf:build`, `cd ~/AssistSupport && pnpm perf:assets`, `cd ~/AssistSupport && pnpm perf:workspace`, `cd ~/AssistSupport && pnpm perf:memory`, `cd ~/AssistSupport && pnpm perf:lhci`, `cd ~/AssistSupport && pnpm git:guard:all`, `cd ~/AssistSupport && ASSISTSUPPORT_DB_HOST=127.0.0.1 ASSISTSUPPORT_DB_PORT=5432 ASSISTSUPPORT_DB_USER=assistsupport_dev ASSISTSUPPORT_DB_NAME=assistsupport_dev pnpm perf:db`, and `cd ~/AssistSupport && BASE_URL=http://127.0.0.1:3000 AUTH_TOKEN=local-smoke-key pnpm perf:api`.
  - The final closeout used temporary local PostgreSQL 16 and Redis via Homebrew services plus a loopback Gunicorn sidecar started from `search-api/venv`. `perf:db` passed via the BM25 `EXPLAIN` fallback with `execution_time_ms=2.776` and `planning_time_ms=44.086`, and `perf:api` passed with `p95=28.5ms`, `p99=100.9ms`, `failureRate=0`, and `checksRate=1`.
  - Whole-program closeout can now be treated as complete. No substantive Batch 13 implementation phase is justified; any future follow-up should be limited to optional release-readiness or maintenance notes.
  - The existing non-blocking Lighthouse SEO warning remains unchanged at `0.82` against the `0.90` threshold.
...
- Tauri command registration, build-manifest generation, and permission parity all need the same source of truth. When those guardrails scrape different files, backend refactors fail for structural reasons before behavior is even exercised.
- Re-exported command paths are not enough for Tauri’s generated command macros. The registry must point at the real command-owning module, even when the public surface is re-exported elsewhere.
- The database monolith already contained usable seam lines. A safe split came from moving method bodies into domain-owned `impl Database` modules while keeping the public `Database`, `DbError`, and record-type import surface stable for one wave.
- SQLite-local query health needs its own Rust smoke coverage in this repo. The existing `pnpm perf:db` lane is aimed at the search-api/Postgres path, so it does not meaningfully guard the desktop app database on its own.
- Cross-module inherent methods that were private inside the monolith may need `pub(crate)` visibility after an internal store split, even when the public facade stays unchanged.
- The desktop app had already converged on adaptive search at the Tauri bridge, so alternate `fusion_strategy` values were compatibility surface only rather than active product behavior.
- The local perf harnesses for the search sidecar need explicit runtime inputs. `perf:api` requires a live loopback base URL, and `perf:db` only proves anything when a reachable Postgres instance with KB search tables is available.
- The whole-codebase Phase 8 review found no open P0/P1 blockers after the final search-api simplification tree was validated end to end.
- Shared UI tab types can lag behind the real shell even after route/render logic is cleaned up. Batch 9 surfaced that drift in the header/sidebar/tab-bar layer, so deletion waves still need one shared-type audit instead of assuming shell cleanup is fully centralized.
- Removing compatibility residue safely is often more about aligning docs, generated contracts, mocks, and helper modules than about changing the live runtime path itself.
- The current `.perf-baselines/*` files still describe the surviving product/runtime shape well enough after the deletion wave, so “baseline reset” should remain a measured decision rather than an automatic batch-close step.
- Broad compatibility files are easiest to retire safely once structural guards exist first. The import-policy test and the retired-legacy-module test let the repo reject backsliding instead of relying on memory.
- The last command-convergence work was mostly ownership cleanup, not product behavior change. That made focused Rust structure tests plus the full repo pack a safer strategy than trying to infer safety from diff size alone.
- The API and DB perf lanes remain real end-to-end checks only when the temporary local loopback services are explicitly started. Recording that setup in the roadmap is part of making the closeout reproducible.
- `src-tauri/src/db/mod.rs` can shrink meaningfully without destabilizing callers when the dead broad store re-exports are pruned first and the stable facade is left intact.
- `model_commands.rs` can be reduced safely when the public Tauri command layer stays put and only the helper-heavy runtime ownership moves into `pub(crate)` submodules.
- Release-readiness docs are only fully trustworthy once they are exercised on a live loopback stack. The final Batch 12 closeout proved the runbook path with Homebrew Postgres/Redis plus a local Gunicorn sidecar.
...
- Batch 6 keeps a temporary `legacy_commands.rs` implementation home behind the new registry and thin index. The command surface and registration ownership are cleaned up in this phase, while the deeper internal logic breakup can continue in later backend waves.
- Batch 7 keeps the public DB record types in `src-tauri/src/db/mod.rs` via compatibility re-exports instead of forcing every caller onto new domain-type paths in the same wave. The store implementations moved; the import surface intentionally stayed stable.
- Batch 8 intentionally keeps compatibility-only search surfaces for one more wave: accepted legacy `fusion_strategy` inputs, the `rerank_time_ms` response field, and offline modules like `reranker.py` and broader score-fusion utilities remain in the repo even though the live runtime no longer depends on them.
- The Phase 8 closeout needed explicit local runtime setup for the last perf commands instead of bare `pnpm perf:api` and `pnpm perf:db` invocations. The recorded verification evidence now captures that setup so the validation is reproducible rather than implied.
- Batch 9 intentionally does not remove `src/types/index.ts` or decompose `src-tauri/src/commands/legacy_commands.rs`. Both remain cleanup targets, but deleting them in the same wave would have expanded a low-regret deletion pass into a deeper frontend/backend migration.
- Batch 11 intentionally keeps the stable `Database` facade even after narrowing `src-tauri/src/db/mod.rs`. The wave removes dead compatibility breadth, but does not force a broader DB API migration.
- Batch 11 also keeps the live search endpoint set unchanged while fully removing `fusion_strategy` from the surviving request path. The contract tightened, but the service shape did not expand or move.
- Batch 12 needed a short follow-up after the merge because the final perf gates were initially environment-blocked. Once local Postgres, Redis, and the Gunicorn sidecar were brought up, the program closed cleanly without further code changes.
...
- Batch 6 command-surface split is being shipped from `codex/refactor/tauri-command-registry-split`.
- Batch 7 database-facade split is being shipped from `codex/refactor/db-domain-facade`.
- Batch 8 search-api simplification is being shipped from `codex/refactor/search-api-simplification`.
- Batch 9 deletion and baseline review is being shipped from `codex/chore/delete-dead-paths`.
- Batch 10 internal convergence groundwork is being shipped from `codex/refactor/internal-convergence-and-final-compatibility-retirement`.
- Batch 11 final convergence and hardening closeout is being shipped from `codex/refactor/internal-convergence-and-final-compatibility-retirement`.
- Batch 12 final module right-sizing and release-readiness cleanup is being shipped from `codex/docs/phase12-closeout`.
- Batch 12 final module right-sizing and release-readiness cleanup is being shipped from `codex/refactor/internal-convergence-and-final-compatibility-retirement`.
