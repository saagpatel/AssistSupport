...
- What is healthy: SQLCipher/key-wrapping and file-permission posture in [src-tauri/src/security.rs#L1](~/Projects/AssistSupport/src-tauri/src/security.rs#L1) and [src-tauri/src/db/mod.rs#L58](~/Projects/AssistSupport/src-tauri/src/db/mod.rs#L58) are comparatively strong; loopback URL validation and SSRF protections are stronger than average in [src-tauri/src/commands/search_api.rs#L129](~/Projects/AssistSupport/src-tauri/src/commands/search_api.rs#L129) and [src-tauri/src/kb/network.rs#L1](~/Projects/AssistSupport/src-tauri/src/kb/network.rs#L1); `pnpm build`, `pnpm ui:gate:static`, `pnpm test`, `pnpm ui:test:a11y`, isolated Python `pytest`, `pip-audit`, and Rust security-regression tests passed.
...
- Explicit non-findings: I did not validate a current SSRF/path-traversal/master-key-storage exploit in shipped paths. The legacy rebinding-vulnerable helper in [src-tauri/src/kb/network.rs#L250](~/Projects/AssistSupport/src-tauri/src/kb/network.rs#L250) is still present, but the audited production callers use the pinned validator, so I am treating that as cleanup, not as a live P1/P2 exploit.
...
- `P1-01` Vector-store isolation, deletion guarantees, and privacy posture are not trustworthy.
  ...
  Evidence: [src-tauri/src/commands/mod.rs#L2454](~/Projects/AssistSupport/src-tauri/src/commands/mod.rs#L2454), [src-tauri/src/kb/vectors.rs#L499](~/Projects/AssistSupport/src-tauri/src/kb/vectors.rs#L499), [src-tauri/src/kb/search.rs#L404](~/Projects/AssistSupport/src-tauri/src/kb/search.rs#L404), [src-tauri/src/commands/mod.rs#L4279](~/Projects/AssistSupport/src-tauri/src/commands/mod.rs#L4279), [src-tauri/src/kb/vectors.rs#L640](~/Projects/AssistSupport/src-tauri/src/kb/vectors.rs#L640), [src-tauri/src/kb/vectors.rs#L712](~/Projects/AssistSupport/src-tauri/src/kb/vectors.rs#L712). Official LanceDB documentation positions encryption-at-rest as an enterprise feature: [LanceDB Enterprise Overview](https://www.lancedb.com/docs/cloud/enterprise/overview/). The “current local store is plaintext” conclusion is an inference supported by that source and local code setting `encryption_enabled: false` in [src-tauri/src/commands/mod.rs#L237](~/Projects/AssistSupport/src-tauri/src/commands/mod.rs#L237).
...
- `P1-02` Search API request handling is not concurrency-safe.
  ...
  Evidence: [search-api/search_api.py#L46](~/Projects/AssistSupport/search-api/search_api.py#L46), [search-api/search_api.py#L54](~/Projects/AssistSupport/search-api/search_api.py#L54), [search-api/search_api.py#L382](~/Projects/AssistSupport/search-api/search_api.py#L382), [search-api/hybrid_search.py#L28](~/Projects/AssistSupport/search-api/hybrid_search.py#L28). Psycopg documents that cursors are not thread-safe and shared connections serialize work: [psycopg2 thread/process safety](https://www.psycopg.org/docs/usage.html#thread-and-process-safety), [psycopg3 concurrency notes](https://www.psycopg.org/psycopg3/docs/advanced/async.html). The production-failure impact is an inference from those sources plus the repo’s threaded Flask configuration.
...
- `P1-03` Search API CI/smoke can pass while the real runtime path is broken.
  ...
  Evidence: [search-api/requirements-test.txt#L1](~/Projects/AssistSupport/search-api/requirements-test.txt#L1), [search-api/requirements.txt#L1](~/Projects/AssistSupport/search-api/requirements.txt#L1), [search-api/tests/test_search_api_endpoints.py#L13](~/Projects/AssistSupport/search-api/tests/test_search_api_endpoints.py#L13), [search-api/smoke_search_api.py#L23](~/Projects/AssistSupport/search-api/smoke_search_api.py#L23), [\.github/workflows/ci.yml#L179](~/Projects/AssistSupport/.github/workflows/ci.yml#L179).
...
- `P1-04` Rust dependency posture contains an active high advisory and stale waiver governance.
  ...
  Evidence: local `cargo audit` reports [RUSTSEC-2026-0037](https://rustsec.org/advisories/RUSTSEC-2026-0037) on `quinn-proto 0.11.13` with fix `>=0.11.14`; current waiver file is [scripts/security/run-cargo-audit.sh#L7](~/Projects/AssistSupport/scripts/security/run-cargo-audit.sh#L7) and is dated 2026-03-01 without the new advisory.
...
- `P2-01` Recovery, backup, and data-migration flows are unsafe or misleading.
  ...
  Evidence: startup integrity path in [src-tauri/src/commands/mod.rs#L142](~/Projects/AssistSupport/src-tauri/src/commands/mod.rs#L142) and [src-tauri/src/db/mod.rs#L112](~/Projects/AssistSupport/src-tauri/src/db/mod.rs#L112); repair command requires initialized DB in [src-tauri/src/commands/diagnostics.rs#L67](~/Projects/AssistSupport/src-tauri/src/commands/diagnostics.rs#L67); backup import/decrypt allocation in [src-tauri/src/backup.rs#L261](~/Projects/AssistSupport/src-tauri/src/backup.rs#L261) and [src-tauri/src/backup.rs#L367](~/Projects/AssistSupport/src-tauri/src/backup.rs#L367); migration conflicts in [src-tauri/src/migration.rs#L168](~/Projects/AssistSupport/src-tauri/src/migration.rs#L168); backup scope in [src-tauri/src/backup.rs#L3](~/Projects/AssistSupport/src-tauri/src/backup.rs#L3) versus UI copy in [src/components/Settings/SettingsTab.tsx#L1497](~/Projects/AssistSupport/src/components/Settings/SettingsTab.tsx#L1497). SQLite documents that `integrity_check` does not detect foreign-key errors, which matters for recovery-mode design: [SQLite PRAGMA docs](https://www.sqlite.org/pragma.html#pragma_integrity_check). The recommendation to add `foreign_key_check` is therefore sourced.
...
- `P2-02` Security initialization and key-storage contracts are misleading and partially broken.
  ...
  Evidence: passphrase/keychain UI in [src/components/shared/OnboardingWizard.tsx#L227](~/Projects/AssistSupport/src/components/shared/OnboardingWizard.tsx#L227), modal flow in [src/components/shared/OnboardingWizard.tsx#L405](~/Projects/AssistSupport/src/components/shared/OnboardingWizard.tsx#L405), init hook continuing into DB checks in [src/hooks/useInitialize.ts#L115](~/Projects/AssistSupport/src/hooks/useInitialize.ts#L115), stale TS contract in [src/types/index.ts#L2](~/Projects/AssistSupport/src/types/index.ts#L2), unconditional availability check in [src-tauri/src/commands/mod.rs#L331](~/Projects/AssistSupport/src-tauri/src/commands/mod.rs#L331), and README claims in [README.md#L3](~/Projects/AssistSupport/README.md#L3) and [README.md#L33](~/Projects/AssistSupport/README.md#L33).
...
- `P2-03` Shell-level resilience and accessibility are incomplete.
  ...
  Evidence: no root boundary in [src/main.tsx#L10](~/Projects/AssistSupport/src/main.tsx#L10) and [src/App.tsx#L185](~/Projects/AssistSupport/src/App.tsx#L185); tab-only boundaries in [src/features/app-shell/renderActiveTab.tsx#L47](~/Projects/AssistSupport/src/features/app-shell/renderActiveTab.tsx#L47]); modal examples in [src/components/Draft/SaveAsTemplateModal.tsx#L42](~/Projects/AssistSupport/src/components/Draft/SaveAsTemplateModal.tsx#L42), [src/components/Settings/SettingsTab.tsx#L1319](~/Projects/AssistSupport/src/components/Settings/SettingsTab.tsx#L1319), [src/components/Sources/SourcesTab.tsx#L358](~/Projects/AssistSupport/src/components/Sources/SourcesTab.tsx#L358), [src/components/FollowUps/FollowUpsTab.tsx#L556](~/Projects/AssistSupport/src/components/FollowUps/FollowUpsTab.tsx#L556); keyboard-hostile KB items in [src/components/Knowledge/KnowledgeBrowser.tsx#L149](~/Projects/AssistSupport/src/components/Knowledge/KnowledgeBrowser.tsx#L149) and [src/components/Knowledge/KnowledgeBrowser.css#L126](~/Projects/AssistSupport/src/components/Knowledge/KnowledgeBrowser.css#L126). The expected fix behavior should follow the [WAI-ARIA modal dialog pattern](https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/).
...
- `P2-04` Search API readiness, deployment assumptions, and operator health signals are too shallow.
  ...
  Evidence: static health response in [search-api/search_api.py#L117](~/Projects/AssistSupport/search-api/search_api.py#L117), narrow runtime validation in [search-api/runtime_config.py#L104](~/Projects/AssistSupport/search-api/runtime_config.py#L104), WSGI entrypoint in [search-api/wsgi.py#L1](~/Projects/AssistSupport/search-api/wsgi.py#L1), path-bootstrap difference versus [search-api/search_api.py#L11](~/Projects/AssistSupport/search-api/search_api.py#L11), and Rust-side health checks only reading `/health` in [src-tauri/src/commands/search_api.rs#L358](~/Projects/AssistSupport/src-tauri/src/commands/search_api.rs#L358). Flask’s own docs recommend a production WSGI/ASGI server rather than the development server: [Flask deployment docs](https://flask.palletsprojects.com/en/stable/deploying/), [Flask development server warning](https://flask.palletsprojects.com/en/stable/server/). The specific cwd-fragility conclusion is local evidence plus one repo-root import failure during this audit.
...
- `P2-05` Trust-boundary governance and system documentation are drifting from reality.
  ...
  Evidence: 227 Tauri commands in the repo; only plugin-default capability in [src-tauri/capabilities/default.json#L1](~/Projects/AssistSupport/src-tauri/capabilities/default.json#L1); no `src-tauri/permissions/` directory; empty generated contract in [openapi/openapi.generated.json](~/Projects/AssistSupport/openapi/openapi.generated.json); docs gate scope in [scripts/ci/require-tests-and-docs.mjs#L17](~/Projects/AssistSupport/scripts/ci/require-tests-and-docs.mjs#L17); ADR template only in [docs/adr/0000-template.md](~/Projects/AssistSupport/docs/adr/0000-template.md); no operational runbooks in [docs](~/Projects/AssistSupport/docs). Tauri documents app-defined permissions/capabilities and runtime authority here: [permissions](https://tauri.app/learn/security/using-plugin-permissions/), [runtime authority](https://tauri.app/develop/calling-rust/). The conclusion that the repo lacks custom least-privilege segmentation is an inference from those docs plus the repo structure.
...
- `P2-06` Regression coverage depth is too thin, and coverage enforcement is uneven across subsystems.
  ...
  Evidence: only three frontend unit-test files and one Playwright spec; `pnpm test` currently covers nine frontend tests; frontend diff coverage only in [\.github/workflows/quality-gates.yml#L56](~/Projects/AssistSupport/.github/workflows/quality-gates.yml#L56); Python endpoint tests fully stub the heavy engine in [search-api/tests/test_search_api_endpoints.py#L13](~/Projects/AssistSupport/search-api/tests/test_search_api_endpoints.py#L13); visual smoke currently fails on [tests/ui/app-shell.spec.ts#L8](~/Projects/AssistSupport/tests/ui/app-shell.spec.ts#L8).
...
- `P3-01` Policy and lifecycle consistency still have cleanup gaps.
  ...
  Evidence: direct vector enable path in [src-tauri/src/commands/mod.rs#L2733](~/Projects/AssistSupport/src-tauri/src/commands/mod.rs#L2733); app abort points in [src-tauri/src/lib.rs#L56](~/Projects/AssistSupport/src-tauri/src/lib.rs#L56), [src-tauri/src/lib.rs#L346](~/Projects/AssistSupport/src-tauri/src/lib.rs#L346), and [src-tauri/src/audit.rs#L466](~/Projects/AssistSupport/src-tauri/src/audit.rs#L466); legacy SSRF helper remains public in [src-tauri/src/kb/network.rs#L250](~/Projects/AssistSupport/src-tauri/src/kb/network.rs#L250) even though production callers use the pinned path.
...
- `P3-02` Workflow hardening, local parity, and performance-gate enforcement need cleanup.
  ...
  Evidence: mixed toolchains in [\.github/workflows/ci.yml#L29](~/Projects/AssistSupport/.github/workflows/ci.yml#L29) and [\.github/workflows/quality-gates.yml#L20](~/Projects/AssistSupport/.github/workflows/quality-gates.yml#L20); mutable action tags in multiple workflows; no `github-actions` entry in [\.github/dependabot.yml](~/Projects/AssistSupport/.github/dependabot.yml); CI-only dist stub in [\.github/workflows/ci.yml#L153](~/Projects/AssistSupport/.github/workflows/ci.yml#L153); performance lanes conditional on repo vars in [\.github/workflows/perf-enforced.yml#L10](~/Projects/AssistSupport/.github/workflows/perf-enforced.yml#L10). GitHub recommends pinning actions to full-length SHAs: [GitHub Actions security hardening](https://docs.github.com/en/actions/how-tos/security-for-github-actions/security-guides/security-hardening-for-github-actions).
...
- `P3-03` Module boundaries, ownership, and minor UX semantics create continuing change risk.
  ...
  Evidence: [src-tauri/src/commands/mod.rs](~/Projects/AssistSupport/src-tauri/src/commands/mod.rs) is 6239 lines, [src-tauri/src/db/mod.rs](~/Projects/AssistSupport/src-tauri/src/db/mod.rs) is 6206 lines, [src/components/Settings/SettingsTab.tsx](~/Projects/AssistSupport/src/components/Settings/SettingsTab.tsx) is 1793 lines, [src/components/Draft/DraftTab.tsx](~/Projects/AssistSupport/src/components/Draft/DraftTab.tsx) is 1285 lines; there is no `CODEOWNERS`; nested `main` landmarks appear in [src/App.tsx#L140](~/Projects/AssistSupport/src/App.tsx#L140) and [src/features/revamp/shell/RevampShell.tsx#L70](~/Projects/AssistSupport/src/features/revamp/shell/RevampShell.tsx#L70); `SourcesTab` starts in a false empty/setup state in [src/components/Sources/SourcesTab.tsx#L21](~/Projects/AssistSupport/src/components/Sources/SourcesTab.tsx#L21); `useDrafts` shares a single loading flag across unrelated operations in [src/hooks/useDrafts.ts#L15](~/Projects/AssistSupport/src/hooks/useDrafts.ts#L15).
