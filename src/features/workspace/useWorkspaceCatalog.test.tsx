...
function makeOps(
  overrides: Partial<WorkspaceOpsClient> = {},
): WorkspaceOpsClient {
  return {
    ...
    listRunbookStepEvidence: vi.fn(
      async () => [] as RunbookStepEvidenceRecord[],
    ),
...
describe("useWorkspaceCatalog", () => {
  ...
  it("bootstraps default templates when guided runbooks are enabled and none exist yet", async () => {
    ...
    expect(result.current.runbookTemplates[0]?.scenario).toBe(
      "security-incident",
    );
  });

  it("hydrates an active legacy-scoped runbook session when the current workspace has none yet", async () => {
    const listRunbookSessions = vi.fn(
      async (_limit?: number, _status?: string, scopeKey?: string) => {
