...
export function useWorkspaceCatalog({
  ...
  const [runbookTemplates, setRunbookTemplates] = useState<
    GuidedRunbookTemplate[]
  >([]);
  const [guidedRunbookSession, setGuidedRunbookSession] =
    useState<GuidedRunbookSession | null>(null);
  const [workspaceCatalogLoading, setWorkspaceCatalogLoading] = useState(false);
  const [runbookSessionSourceScopeKey, setRunbookSessionSourceScopeKey] =
  ...
  const refreshWorkspaceCatalog = useCallback(async () => {
    ...
    try {
      ...
      setGuidedRunbookSession(
        toGuidedRunbookSession(activeSessionRecord, evidenceRecords),
      );
