...
function makeParams(
  overrides: Partial<Parameters<typeof useWorkspaceDraftState>[0]> = {},
) {
  return {
    workspacePersonalizationStorageKey:
      "assistsupport.workspace.personalization.test",
...
describe("useWorkspaceDraftState", () => {
  it("persists workspace personalization and applies a loaded draft immediately when the workspace is empty", async () => {
    ...
    const draft = makeDraft({
      ...
      kb_sources_json: JSON.stringify([
        { title: "VPN runbook", file_path: "vpn.md" },
      ]),
    });
    ...
    expect(
      localStorage.getItem("assistsupport.workspace.personalization.test"),
    ).toContain('"preferred_note_audience":"internal-note"');
    expect(params.setChecklistCompleted).toHaveBeenCalledWith({
      "step-1": true,
    });
  ...
  it("creates or updates autosave identity and migrates runbook scope when an autosave-only workspace becomes active", async () => {
    ...
    await waitFor(() => {
      expect(reassignRunbookSessionScope).toHaveBeenCalledWith(
        "workspace:test",
        "draft:autosave-1",
      );
    });
