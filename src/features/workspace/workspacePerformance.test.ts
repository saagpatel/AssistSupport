...
function createDraft(index: number): SavedDraft {
  ...
  return {
    ...
    handoff_summary:
      "Escalated to network operations after confirming west-region scope.",
...
function writeWorkspaceLogicResults(payload: Record<string, number | string>) {
  ...
  writeFileSync(
    ".perf-results/workspace-logic.json",
...
describe("workspace performance targets", () => {
  it("keeps similar-case lookup and next-action generation inside roadmap budgets", () => {
    ...
    const similarCaseP95Ms = Number(
      percentile(similarCaseLatencies, 95).toFixed(2),
    );
    const nextActionP95Ms = Number(
      percentile(nextActionLatencies, 95).toFixed(2),
    );
