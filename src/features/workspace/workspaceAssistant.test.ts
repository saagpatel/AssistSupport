...
function makeDraft(partial: Partial<SavedDraft> = {}): SavedDraft {
  return {
    ...
    input_text:
      partial.input_text ?? "VPN disconnects every morning for remote users",
    ...
    response_text:
      ...
      "Reset the VPN profile and verify MFA enrollment.",
...
describe("workspaceAssistant", () => {
    ...
    const similar = buildSimilarCases({
      ...
      drafts: [
        makeDraft({
          ...
          input_text:
            "Remote users report VPN disconnects every morning after sign-in",
        }),
        makeDraft({
          id: "old-2",
          status: "draft",
  ...
  it("builds evidence and KB drafts from the active workspace state", () => {
    ...
    expect(
      evidence.sections.some(
        (section) => section.label === "Current Handoff Pack",
      ),
    ).toBe(true);
  ...
  it("builds and applies reusable resolution kits from the workspace", () => {
    const draft = makeDraft({
      response_text: "Reset the VPN profile and re-enroll MFA.",
    });
    const handoff = buildHandoffPack({
      inputText: draft.input_text,
      responseText: draft.response_text ?? "",
      intake: {
        issue: "VPN disconnects every morning",
        impact:
          "Remote users cannot connect for the first 15 minutes of the day",
        affected_system: "VPN gateway",
        steps_tried: "Reset VPN profile",
      ...
      diagnosticNotes:
        "Escalated to network engineering for west region gateway checks.",
      sources: [
        {
          chunk_id: "chunk-1",
          document_id: "doc-1",
