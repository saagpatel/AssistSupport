...
export function useWorkspaceDraftState({
  ...
  const [pendingDraftOpen, setPendingDraftOpen] = useState<SavedDraft | null>(null);
  ...
  const applyLoadedDraft = useCallback((draft: SavedDraft) => {
    ...
    if (draft.diagnosis_json) {
      try {
        const diagData = JSON.parse(draft.diagnosis_json) as {
          ...
          trust?: { confidence?: ConfidenceAssessment | null; grounding?: GroundedClaim[] } | null;
        };
  ...
  const requestOpenSimilarCase = useCallback((similarCase: SimilarCase) => {
