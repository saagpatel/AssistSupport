...
vi.mock('../../../hooks/useCustomVariables', () => ({
  useCustomVariables: () => ({
    variables: [{ id: 'custom-1', name: 'team_name', value: 'Network Operations' }],
...
function makeTemplate(partial: Partial<ResponseTemplate> = {}): ResponseTemplate {
  return {
...
describe('QueueHistoryTemplatesPanel', () => {
  beforeEach(() => {
    ...
    getDraftMock.mockImplementation(async (draftId: string) => draftsState.find((draft) => draft.id === draftId) ?? null);
  ...
  it('renders the controlled templates section without showing the internal section tabs', () => {
    ...
    render(<QueueHistoryTemplatesPanel activeSection="templates" hideSectionTabs />);
  ...
  it('filters draft history and deletes the selected draft after confirmation', async () => {
    draftsState = [
      makeDraft({ id: 'draft-1', ticket_id: 'INC-1001', summary_text: 'VPN outage' }),
      makeDraft({
        id: 'draft-2',
        ticket_id: 'INC-1002',
    ...
    fireEvent.change(screen.getByPlaceholderText('Filter by ticket...'), { target: { value: 'INC-1002' } });
    expect(await screen.findByRole('button', { name: 'INC-1002' })).toBeTruthy();
    expect(screen.queryByRole('button', { name: 'INC-1001' })).toBeNull();

    fireEvent.change(screen.getByPlaceholderText('Search drafts...'), { target: { value: 'no-match' } });
    expect(await screen.findByText(/No drafts found/)).toBeTruthy();

    fireEvent.change(screen.getByPlaceholderText('Search drafts...'), { target: { value: '' } });
    expect(await screen.findByRole('button', { name: 'INC-1002' })).toBeTruthy();
  ...
  it('restores an older version and opens the restored draft in the workspace callback', async () => {
    ...
    const currentDraft = makeDraft({ id: 'draft-1', response_text: 'Current response' });
    ...
    getDraftMock.mockResolvedValue({ ...currentDraft, response_text: 'Restored response' });
    ...
    await waitFor(() => {
      expect(restoreDraftVersionMock).toHaveBeenCalledWith('draft-1', 'draft-older');
      expect(onLoadDraft).toHaveBeenCalledWith(expect.objectContaining({ response_text: 'Restored response' }));
    });
  });
  ...
  it('inserts template variables during template creation and saves the new template content', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Name'), { target: { value: 'Customer update' } });
