...
export function buildAppShellCommands({
  ...
  const commands: Array<Command | null> = [
    ...
    makeWorkspaceEventCommand(
      ...
      revampFlags.ASSISTSUPPORT_SIMILAR_CASES,
      {
        ...
        description:
          "Compare the current draft to the best similar solved case",
        icon: "sparkles",
        category: "draft",
      },
    ),
    makeWorkspaceEventCommand(WORKSPACE_COPY_HANDOFF_EVENT, true, {
    ...
    makeWorkspaceEventCommand(WORKSPACE_COPY_EVIDENCE_EVENT, true, {
    ...
    makeWorkspaceEventCommand(WORKSPACE_COPY_KB_DRAFT_EVENT, true, {
  ...
  const filtered: Command[] = commands.filter((cmd): cmd is Command =>
    Boolean(cmd),
  );
