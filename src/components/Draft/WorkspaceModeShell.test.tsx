...
describe('WorkspaceModeShell', () => {
  ...
  it('renders workflow, panels, and dialogs in panel mode with the expected shell class names', () => {
    ...
    expect((container.firstChild as HTMLElement).className).toContain('draft-tab');
    expect((container.firstChild as HTMLElement).className).toContain('panel-density-focus-intake');
    expect((container.firstChild as HTMLElement).className).toContain('diagnosis-collapsed');
    expect((container.firstChild as HTMLElement).className).toContain('has-workspace-rail');
  });
});
