...
vi.mock('./SaveAsTemplateModal', () => ({
  SaveAsTemplateModal: ({ content }: { content: string }) => <div>Template modal:{content}</div>,
}));
...
describe('WorkspaceDialogs', () => {
  ...
  it('keeps compare disabled without a response and routes the other confirm actions correctly', () => {
    ...
    expect((screen.getByRole('button', { name: 'Compare instead' }) as HTMLButtonElement).disabled).toBe(true);

    const saveAndOpenButtons = screen.getAllByRole('button', { name: 'Save and open' });
    const openAnywayButtons = screen.getAllByRole('button', { name: 'Open anyway' });
