...
export function SettingsHero({
  ...
  return (
    <header className="settings-hero" aria-label="Settings overview">
      ...
      <div
        className="settings-hero__pills"
        aria-label="System readiness summary"
      >
        <span
          className={[
            "settings-pill",
            loadedModel ? "is-good" : "is-warn",
          ].join(" ")}
        >
          LLM: {loadedModel ? "Loaded" : "Not loaded"}
        </span>
        <span
          className={["settings-pill", kbFolder ? "is-good" : "is-warn"].join(
            " ",
          )}
        >
        ...
        <span
          className={[
            "settings-pill",
            isEmbeddingLoaded ? "is-good" : "is-warn",
          ].join(" ")}
        >
          Embeddings:{" "}
          {isEmbeddingLoaded
  ...
  return (
    <section className="settings-section" aria-label="Policy gates">
      ...
      <p className="settings-description">
        These switches control whether potentially sensitive UI surfaces can
        appear. Outside development builds, policy flags are
        environment-variable authoritative (local overrides are ignored).
      </p>
      <div className="settings-grid">
        <div className="settings-card">
          ...
          <ul className="settings-list">
            ...
            <li>
              <strong>Enable:</strong> set{" "}
              <code>VITE_ASSISTSUPPORT_ENABLE_ADMIN_TABS=1</code>
            </li>
            <li>
        ...
        <div className="settings-card">
          ...
          <ul className="settings-list">
            ...
            <li>
              <strong>Enable (UI):</strong> set{" "}
              <code>VITE_ASSISTSUPPORT_ENABLE_NETWORK_INGEST=1</code>
            </li>
            <li>
              <strong>Enable (backend):</strong> set{" "}
              <code>ASSISTSUPPORT_ENABLE_NETWORK_INGEST=1</code>
            </li>
            <li>
              <strong>Default:</strong> disabled
            </li>
...
export function MemoryKernelSection({
  ...
  return (
    <section className="settings-section" aria-label="MemoryKernel integration">
      <h2>MemoryKernel</h2>
      <p className="settings-description">
        Optional local enrichment. If unavailable, AssistSupport keeps running
        with deterministic fallback and no runtime cutover.
      </p>
      <div className="settings-grid">
        <div className="settings-card">
          ...
          <ul className="settings-list">
            <li>
              ...
              {memoryKernelPreflight?.enabled ? "Yes" : "No"}
            </li>
            <li>
              <strong>Ready:</strong>{" "}
              {memoryKernelPreflight?.ready ? "Yes" : "No"}
            </li>
            <li>
              <strong>Enrichment:</strong>{" "}
              {memoryKernelPreflight?.enrichment_enabled
                ? "Enabled"
                : "Disabled"}
            </li>
            <li>
              <strong>Base URL:</strong>{" "}
              {memoryKernelPreflight?.base_url ? (
                <code>{memoryKernelPreflight.base_url}</code>
              ) : (
                "Unavailable"
              )}
            </li>
        ...
        <div className="settings-card">
          ...
          <ul className="settings-list">
            <li>
              ...
              {memoryKernelPreflight ? (
                <>
                  <code>{memoryKernelPreflight.release_tag}</code> ·{" "}
                  <code>{memoryKernelPreflight.commit_sha}</code>
                </>
              ) : (
                "Unavailable"
              )}
            ...
            <li>
              ...
              (expected{" "}
              <code>
                {memoryKernelPreflight?.expected_service_contract_version ??
                  "—"}
              </code>
              )
            </li>
            <li>
              ...
              (expected{" "}
              <code>
                {memoryKernelPreflight?.expected_api_contract_version ?? "—"}
              </code>
              )
            </li>
            <li>
              ...
              {memoryKernelPreflight ? (
                <code>{memoryKernelPreflight.integration_baseline}</code>
              ) : (
                "Unavailable"
              )}
            </li>
