...
export function OpsTab() {
  ...
  const [verification, setVerification] =
    useState<SignedArtifactVerificationResult | null>(null);
  ...
  const [integrationConfigDraft, setIntegrationConfigDraft] = useState<
    Record<string, string>
  >({});
  const [integrationBusyType, setIntegrationBusyType] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    setOpsError(null);
    void Promise.all([refreshDeployment(), refreshIntegrations()]).catch(
      (error) => {
        setOpsError(
          typeof error === "string"
            ? error
            : "Failed to load operations diagnostics",
        );
      },
    );
      ...
      <header className="ops-header">
        <div>
          ...
          <p className="ops-subtitle">
            Internal deployment diagnostics and local integration controls.
            Eval, triage, and runbook tools stay out of the active UI in this
            wave.
          </p>
        </div>
      </header>
      ...
      {view === "deployment" && (
        <section className="ops-section" aria-label="Deployment diagnostics">
          <div className="ops-section-copy">
            ...
            <p>
              Run preflight checks, review recorded artifacts, verify
              signatures, and mark the last run for rollback when needed.
            </p>
          </div>
          ...
          <div className="ops-row">
            ...
            <Button
              ...
              disabled={
                ...
                deploymentHealth.last_run.status === "rolled_back"
              }
              loading={deployBusy}
            >
              Roll Back Last Run
          ...
          {deploymentHealth ? (
            <div className="ops-kpis">
              ...
              <div>
                Last status: {deploymentHealth.last_run?.status ?? "none"}
              </div>
            </div>
          ) : (
            <div className="ops-empty">
              Deployment health is not available yet.
            </div>
          )}
          ...
          <div className="ops-grid">
            <input
              ...
              onChange={(e) =>
                setArtifactForm((p) => ({ ...p, artifactType: e.target.value }))
              }
            ...
            <input
              ...
              onChange={(e) =>
                setArtifactForm((p) => ({ ...p, version: e.target.value }))
              }
              placeholder="version"
            />
            <input
              className="ops-input"
              value={artifactForm.channel}
              onChange={(e) =>
                setArtifactForm((p) => ({ ...p, channel: e.target.value }))
              }
              placeholder="channel"
            />
            <input
              className="ops-input"
              value={artifactForm.sha256}
              onChange={(e) =>
                setArtifactForm((p) => ({ ...p, sha256: e.target.value }))
              }
              placeholder="sha256"
            />
            <label className="ops-checkbox">
              <input
                type="checkbox"
                checked={artifactForm.isSigned}
                onChange={(e) =>
                  setArtifactForm((p) => ({ ...p, isSigned: e.target.checked }))
                }
              />{" "}
              Signed
            </label>
            <Button
              ...
              disabled={!artifactForm.sha256.trim()}
              loading={deployBusy}
            >
              Record Artifact
            </Button>
          ...
          <div className="ops-card-list">
            {artifacts.length === 0 && (
              <div className="ops-empty">
                No deployment artifacts recorded yet.
              </div>
            )}
            {artifacts.map((artifact) => (
              <div key={artifact.id} className="ops-card">
                ...
                <div className="ops-card-meta">
                  {artifact.channel} •{" "}
                  {artifact.is_signed ? "Signed" : "Unsigned"}
                </div>
      ...
      {view === "integrations" && (
        <section className="ops-section" aria-label="Integration diagnostics">
          <div className="ops-section-copy">
            <h3>Integrations</h3>
            <p>
              Review local integration enablement and adjust stored config
              objects for supported destinations.
            </p>
          </div>
              ...
              return (
                <div key={type} className="ops-card">
                  ...
                  <textarea
                    ...
                    onChange={(event) =>
                      setIntegrationConfigDraft((current) => ({
                        ...current,
                  ...
                  <div className="ops-row">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      onClick={() =>
                        saveIntegration(type, Boolean(config?.enabled))
                      }
                      loading={isBusy}
                    >
                      Save Config
      ...
      {showRollbackConfirm && (
        <div className="ops-modal-overlay" role="presentation">
          ...
          >
            ...
            <p>
              This marks the latest deployment run as rolled back and keeps the
              reason in the local diagnostics log.
            </p>
            <div className="ops-row">
              <Button
                variant="secondary"
