...
describe('OpsTab', () => {
  it('shows only deployment and integrations in the active UI navigation', async () => {
    ...
    expect(screen.getByText('Deployment health is not available yet.')).toBeTruthy();
    expect(screen.getByText('No deployment artifacts recorded yet.')).toBeTruthy();
  });

  it('keeps the record artifact action disabled until a sha is provided', async () => {
    render(<OpsTab />);

    const recordButton = await screen.findByRole('button', { name: 'Record Artifact' });
    expect((recordButton as HTMLButtonElement).disabled).toBe(true);
    fireEvent.change(screen.getByPlaceholderText('sha256'), { target: { value: 'abc123' } });
    expect((screen.getByRole('button', { name: 'Record Artifact' }) as HTMLButtonElement).disabled).toBe(false);
  });
  ...
  it('shows integration controls and surfaces config errors through the toast channel', async () => {
    ...
    const [textarea] = await screen.findAllByPlaceholderText('{"webhook_url":"https://..."}');
