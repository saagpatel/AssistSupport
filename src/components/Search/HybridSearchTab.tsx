      ...
      <div className="hybrid-search-header">
        <div className="hybrid-search-title-row">
          ...
          <div className="hybrid-search-status">
            <span
              className={`api-status-dot ${apiHealthy === true ? "healthy" : apiHealthy === false ? "unhealthy" : "unknown"}`}
            />
        ...
        {apiStatusMessage && (
          <p
            className={`api-status-detail ${apiHealthy === false ? "unhealthy" : ""}`}
          >
            {apiStatusMessage}
          </p>
        )}
          ...
          <div className="stats-grid">
            ...
            <div className="stat-card">
              <span className="stat-value">
                {stats.latency_ms.avg.toFixed(0)}ms
              </span>
              <span className="stat-label">Avg Latency</span>
            </div>
            <div className="stat-card">
              <span className="stat-value">
                {stats.latency_ms.p95.toFixed(0)}ms
              </span>
            ...
            <div className="stat-card">
              <span className="stat-value">
                {Object.keys(stats.intent_distribution).length}
              </span>
        ...
        <div className="hybrid-search-results">
          <div className="results-meta">
            <span className="results-count">
              {response.results_count} results
            </span>
            <span
              className="results-intent"
              style={{
                color:
                  CATEGORY_COLORS[response.intent.toUpperCase()] ??
                  "var(--text-secondary)",
                backgroundColor:
                  CATEGORY_BG[response.intent.toUpperCase()] ??
                  "var(--bg-tertiary)",
              }}
            >
              {response.intent}
...
function ResultCard({ result, currentRating, onFeedback }: ResultCardProps) {
  const categoryColor =
    CATEGORY_COLORS[result.category] ?? "var(--text-secondary)";
  ...
  return (
    <div className="result-card">
      ...
      {result.section && <div className="result-section">{result.section}</div>}
      ...
      {result.scores && (
        <div className="result-scores">
          ...
          <span className="score">
            Vector: {result.scores.vector.toFixed(3)}
          </span>
          <span className="score score-fused">
            Fused: {result.scores.fused.toFixed(3)}
          </span>
        </div>
      )}
      ...
      <div className="result-feedback">
        {(["helpful", "not_helpful", "incorrect"] as const).map((rating) => (
          <button
            ...
            title={
              rating === "helpful"
                ? "This result was helpful"
                : rating === "not_helpful"
                  ? "This result was not helpful"
