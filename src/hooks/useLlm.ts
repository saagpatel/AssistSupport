...
export function useLlm() {
  ...
  const generateFirstResponse = useCallback(
    async (params: FirstResponseParams): Promise<FirstResponseResult> => {
      setState((prev) => ({ ...prev, generating: true, error: null }));
      try {
        const result = await invoke<FirstResponseResult>(
  ...
  const generateChecklist = useCallback(
    async (params: ChecklistGenerateParams): Promise<ChecklistResult> => {
      setState((prev) => ({ ...prev, generating: true, error: null }));
      try {
        const result = await invoke<ChecklistResult>(
  ...
  const updateChecklist = useCallback(
    async (params: ChecklistUpdateParams): Promise<ChecklistResult> => {
      setState((prev) => ({ ...prev, generating: true, error: null }));
      try {
        const result = await invoke<ChecklistResult>(
          "update_troubleshooting_checklist",
          {
            params,
          },
        );
  ...
  const validateGgufFile = useCallback(
    async (modelPath: string): Promise<GgufFileInfo> => {
      try {
        const info = await invoke<GgufFileInfo>("validate_gguf_file", {
          modelPath,
        });
        return info;
      } catch (e) {
