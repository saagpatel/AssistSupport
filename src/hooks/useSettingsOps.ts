...
export function useSettingsOps(): SettingsOpsClient {
  const getDeploymentHealthSummary =
    useCallback(async (): Promise<DeploymentHealthSummary> => {
      return invoke<DeploymentHealthSummary>("get_deployment_health_summary");
    }, []);

  const listDeploymentArtifacts = useCallback(
    async (limit = 50): Promise<DeploymentArtifactRecord[]> => {
      return invoke<DeploymentArtifactRecord[]>("list_deployment_artifacts", {
        limit,
      });
    },
    [],
  ...
  const runDeploymentPreflight = useCallback(
    async (targetChannel: string): Promise<DeploymentPreflightResult> => {
      return invoke<DeploymentPreflightResult>("run_deployment_preflight", {
        targetChannel,
      });
    },
    [],
  ...
  const recordDeploymentArtifact = useCallback(
    async (
      artifactType: string,
      version: string,
      channel: string,
  ...
  const verifySignedArtifact = useCallback(
    async (
      artifactId: string,
      expectedSha256?: string,
    ): Promise<SignedArtifactVerificationResult> => {
      return invoke<SignedArtifactVerificationResult>(
  ...
  const rollbackDeploymentRun = useCallback(
  ...
  const runEvalHarness = useCallback(
    async (
      suiteName: string,
      cases: EvalHarnessCase[],
    ): Promise<EvalHarnessResult> => {
      return invoke<EvalHarnessResult>("run_eval_harness", {
        suiteName,
        cases,
      });
    },
  ...
  const listEvalRuns = useCallback(
    async (limit = 50): Promise<EvalRunRecord[]> => {
      return invoke<EvalRunRecord[]>("list_eval_runs", { limit });
    },
    [],
  );

  const listIntegrations = useCallback(async (): Promise<
    IntegrationConfigRecord[]
  > => {
