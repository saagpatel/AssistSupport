...
import type { ResolutionKit, WorkspaceFavorite } from "../types/workspace";
import type {
  CaseOutcomeRecord,
  ResolutionKitRecord,
...
export interface WorkspaceOpsClient {
  ...
  saveCaseOutcome: (
    outcome: Omit<CaseOutcomeRecord, "id" | "created_at" | "updated_at"> & {
      id?: string;
    },
  ) => Promise<string>;
...
export function useWorkspaceOps(): WorkspaceOpsClient {
  const listResolutionKits = useCallback(
    async (limit = 50): Promise<ResolutionKitRecord[]> => {
      return invoke<ResolutionKitRecord[]>("list_resolution_kits", { limit });
    },
    [],
  );

  const saveResolutionKit = useCallback(
    async (
      kit: Omit<ResolutionKit, "id"> & { id?: string },
    ): Promise<string> => {
  ...
  const listWorkspaceFavorites = useCallback(async (): Promise<
    WorkspaceFavoriteRecord[]
  > => {
    return invoke<WorkspaceFavoriteRecord[]>("list_workspace_favorites");
  }, []);

  const saveWorkspaceFavorite = useCallback(
    async (
      favorite: Omit<WorkspaceFavorite, "id"> & { id?: string },
    ): Promise<string> => {
      return invoke<string>("save_workspace_favorite", {
        favorite: {
          ...
          metadata_json: favorite.metadata
  ...
  const deleteWorkspaceFavorite = useCallback(
  ...
  const listRunbookTemplates = useCallback(
    async (limit = 50): Promise<RunbookTemplateRecord[]> => {
      return invoke<RunbookTemplateRecord[]>("list_runbook_templates", {
        limit,
      });
    },
    [],
  ...
  const saveRunbookTemplate = useCallback(
    async (
      template: Omit<
        RunbookTemplateRecord,
        "id" | "created_at" | "updated_at"
  ...
  const startRunbookSession = useCallback(
    async (
      scenario: string,
      steps: string[],
      scopeKey: string,
    ): Promise<RunbookSessionRecord> => {
      return invoke<RunbookSessionRecord>("start_runbook_session", {
        scenario,
        steps,
        scopeKey,
      });
  ...
  const advanceRunbookSession = useCallback(
    async (
      sessionId: string,
      currentStep: number,
      status?: string,
  ...
  const listRunbookSessions = useCallback(
    async (
      limit = 50,
      status?: string,
      scopeKey?: string,
  ...
  const reassignRunbookSessionScope = useCallback(
    async (fromScopeKey: string, toScopeKey: string): Promise<void> => {
  ...
  const reassignRunbookSessionById = useCallback(
    async (sessionId: string, toScopeKey: string): Promise<void> => {
  ...
  const listRunbookStepEvidence = useCallback(
    async (sessionId: string): Promise<RunbookStepEvidenceRecord[]> => {
      return invoke<RunbookStepEvidenceRecord[]>("list_runbook_step_evidence", {
        sessionId,
      });
    },
    [],
  ...
  const addRunbookStepEvidence = useCallback(
    async (
      sessionId: string,
      stepIndex: number,
      status: RunbookStepEvidenceRecord["status"],
  ...
  const saveCaseOutcome = useCallback(
    async (
      outcome: Omit<CaseOutcomeRecord, "id" | "created_at" | "updated_at"> & {
        id?: string;
      },
  ...
  const listCaseOutcomes = useCallback(
    async (limit = 50): Promise<CaseOutcomeRecord[]> => {
