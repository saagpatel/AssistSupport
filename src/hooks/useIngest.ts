...
export function useIngest() {
  ...
  const loadDocuments = useCallback(
    async (namespaceId?: string, sourceId?: string) => {
      try {
  ...
  const getDocumentChunks = useCallback(
    async (documentId: string): Promise<DocumentChunk[]> => {
      try {
        return await invoke<DocumentChunk[]>("get_document_chunks", {
          documentId,
        });
      } catch (e) {
        setState((prev) => ({ ...prev, error: String(e) }));
  ...
  const clearKnowledgeData = useCallback(
  ...
  const ingestUrl = useCallback(
    async (url: string, namespaceId: string): Promise<IngestResult> => {
  ...
  const ingestYoutube = useCallback(
    async (url: string, namespaceId: string): Promise<IngestResult> => {
      setState((prev) => ({ ...prev, ingesting: true, error: null }));
      try {
  ...
  const ingestGithub = useCallback(
    async (repoPath: string, namespaceId: string): Promise<IngestResult[]> => {
      setState((prev) => ({ ...prev, ingesting: true, error: null }));
      try {
  ...
  const ingestGithubRemote = useCallback(
    async (repoUrl: string, namespaceId: string): Promise<IngestResult[]> => {
      setState((prev) => ({ ...prev, ingesting: true, error: null }));
      try {
  ...
  const setGithubToken = useCallback(
    async (host: string, token: string): Promise<void> => {
  ...
  const processSourceFile = useCallback(
    async (filePath: string): Promise<BatchIngestResult> => {
      setState((prev) => ({ ...prev, ingesting: true, error: null }));
      try {
  ...
  const getSourceHealth = useCallback(
    async (namespaceId?: string): Promise<SourceHealthSummary> => {
      try {
  ...
  const retrySource = useCallback(
    async (sourceId: string): Promise<IngestResult> => {
      setState((prev) => ({ ...prev, ingesting: true, error: null }));
      try {
  ...
  const markStaleSources = useCallback(
    async (daysThreshold?: number): Promise<number> => {
      try {
