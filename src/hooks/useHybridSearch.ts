...
export function useHybridSearch() {
  ...
  const search = useCallback(
    async (query: string, topK = 10): Promise<HybridSearchResponse | null> => {
      ...
      try {
        ...
        setState((prev) => ({
          ...
          apiStatusMessage: msg.includes("unavailable")
            ? msg
            : prev.apiStatusMessage,
        }));
        return null;
      }
    },
  ...
  const checkHealth = useCallback(async (): Promise<boolean> => {
    try {
      const health = await invoke<SearchApiHealthStatus>(
