...
export interface QueueOpsClient {
  clusterTicketsForTriage: (tickets: TriageTicketInput[]) => Promise<TriageClusterOutput[]>;
  listRecentTriageClusters: (limit?: number) => Promise<TriageClusterRecord[]>;
  previewCollaborationDispatch: (preview: CollaborationDispatchInput) => Promise<DispatchHistoryRecord>;
  confirmCollaborationDispatch: (dispatchId: string) => Promise<DispatchHistoryRecord>;
  cancelCollaborationDispatch: (dispatchId: string) => Promise<DispatchHistoryRecord>;
  listDispatchHistory: (limit?: number, status?: DispatchHistoryRecord['status']) => Promise<DispatchHistoryRecord[]>;
}
