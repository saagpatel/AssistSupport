...
Use the local Codex review flow when you want PR-style feedback without GitHub Actions or repo API secrets:

- `scripts/git/run-codex-review.sh --base origin/main`
- `scripts/git/run-codex-review.sh --uncommitted --base origin/main`
- `ASSISTSUPPORT_REVIEW_PROFILE=fast_review scripts/git/run-codex-review.sh --base origin/main`

This path uses your local Codex login/session on your machine. It does not depend on GitHub workflow secrets.
If the installed `codex-cli` rejects custom prompts for `codex review`, the helper falls back to the built-in review engine so the workflow still runs.
The helper defaults to the `coding` profile and a low review effort for faster branch checks; set `ASSISTSUPPORT_REVIEW_PROFILE=fast_review` for a lighter pass, set `ASSISTSUPPORT_REVIEW_EFFORT=medium` or `high` when you want a deeper pass, set `ASSISTSUPPORT_REVIEW_MODEL` to override the model directly, or set `ASSISTSUPPORT_REVIEW_TIMEOUT_SECS` if you want a specific local review window. The current `codex review` command does not accept profile flags directly, so the helper translates `coding` to `gpt-5-codex` and `fast_review` to `gpt-5-codex-mini` under the hood. For larger diffs it now auto-scales the timeout window and saves a small metadata file alongside the raw stdout/stderr logs under `.codex/reports/local-review/`.
