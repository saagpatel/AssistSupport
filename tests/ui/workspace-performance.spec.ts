...
function writeWorkspaceUiResults(payload: Record<string, number | string>) {
  ...
  writeFileSync(".perf-results/workspace-ui.json", `${JSON.stringify(payload, null, 2)}\n`);
}
...
test("@perf workspace flows stay inside roadmap budgets", async ({ page }) => {
  ...
  await page.waitForFunction(() => {
    const perfState = (window as Window & {
  ...
  const workspaceReadyMs = await page.evaluate((measureName) => {
    ...
    return perfState?.ticketWorkspaceReadyMs ?? (entry ? Number(entry.duration.toFixed(2)) : null);
  ...
  const triageSeed = Array.from({ length: 25 }, (_, index) => {
    ...
    const issueFamily = index % 5 === 0 ? "VPN timeout" : index % 3 === 0 ? "Access approval" : "Printer outage";
  ...
  await expect(page.locator(".as-queue__pre").filter({ hasText: "Tickets:" })).toContainText("Tickets:", {
    timeout: 20_000,
  });
